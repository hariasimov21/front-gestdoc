...
import { getSignedUrlsForLocal } from '@/app/locales/actions';
  ...
  useEffect(() => {
    if (isOpen) {
      const fetchDocuments = async () => {
        ...
        const result = await getSignedUrlsForLocal(local.id_local);
        if (result.error) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-none w-[95vw] h-[95vh] flex flex-col p-0">
        <DialogHeader className="p-4 pb-0">
          ...
          <DialogDescription>
            Local: <span className="font-semibold">{local.nombre_local}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="p-4 border-t">
          <Button asChild variant="outline" disabled={!selectedDocument}>
            <a
              ...
              download={selectedDocument?.nombre_documento}
              target="_blank"
